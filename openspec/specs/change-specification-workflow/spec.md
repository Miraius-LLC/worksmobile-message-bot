...
worksmobile-message-botはOpenSpec 1.13.1をexact devDependencyとして保持し、repository scriptからtelemetryを無効化してstrict validationを実行できなければならない（SHALL）。

#### Scenario: 仕様を検証する

- **WHEN** 開発者、pre-push hook、またはCIが`bun run spec:validate`を実行する
- **THEN** repository-localのOpenSpec 1.13.1がtelemetry無効で全active changeとcurrent specをstrict validationする
