...
type LoggerDestination = { write(line: string): void }

export type Logger = {
  failure: (message: unknown, option?: LogOption) => void
  error: (message: unknown, option?: LogOption) => void
  warn: (message: unknown, option?: LogOption) => void
  info: (message: unknown, option?: LogOption) => void
  success: (message: unknown, option?: LogOption) => void
  request: (message: unknown, option?: LogOption) => void
  debug: (message: unknown, option?: LogOption) => void
}

export function createLogger(
  opts: { pretty?: boolean; destination?: LoggerDestination } = {},
): Logger {
  const pinoLogger = pino(
    ...
    opts.destination as pino.DestinationStream | undefined,
  )
...
/**
 * pino-pretty を使うかどうかの判定。
 * pino-pretty は devDependency なので本番イメージには入っておらず、
 * production で `LOG_PRETTY=1` が誤って入っても無視して JSON 出力に倒す。
 */
export function shouldUsePretty(env: NodeJS.ProcessEnv = process.env): boolean {
  return env['LOG_PRETTY'] === '1' && env['NODE_ENV'] !== 'production'
}
...
export type { LogLevel, LogOption }
