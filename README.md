...
- 検証 OK → dedup チェック (下記) → JSON.parse → Zod の `discriminatedUnion` で event 形式チェック → dispatcher へ流して `200` を返す
...
- **失敗時 retry**: `dispatch` が throw した場合は dedup key を `unregister` して LINE WORKS の再送を許可する。`unregister` を呼ばないと dispatch 失敗 (例: 業務 API への書き込み失敗) の event が再送されても skip されて副作用喪失する
- **検証順序**: 署名検証 → dedup → JSON parse → Zod 検証 → dispatch (副作用)
